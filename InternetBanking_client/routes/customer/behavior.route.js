...
router.route("/transferAboard").post(async function (req, res) {
	...
	switch (data.bank_company_id) {
		...
		case "7APW008iv5sSF1EWskRd":
			...
			verify = hoaKey.verify(JSON.stringify(response.payload  + response.timeStamp), response.signNature, "utf8", "base64");
			break;
	}
...
router.route("/create_customer_account").post(async function(req, res){
